...
def demo_kruskal(graph_name: str, G: Graph):
    ...
    print(
        f"  {checkmark} Number of edges in MST: {len(mst_edges)} (expected: {expected_edges})"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"  {checkmark} Test 1 (Small Graph): {'PASS' if test1_pass else 'FAIL'} - MST Weight: {mst1_weight}"
    )
    print(
        f"  {checkmark} Test 2 (Larger Graph): {'PASS' if test2_pass else 'FAIL'} - MST Weight: {mst2_weight}"
    )
    print(
        f"  {checkmark} Test 3 (Minimal Graph): {'PASS' if test3_pass else 'FAIL'} - MST Weight: {mst3_weight}"
    )
    print(
        f"  {checkmark} Test 4 (Linear Graph): {'PASS' if test4_pass else 'FAIL'} - MST Weight: {mst4_weight}"
    )

    print("\n" + "=" * 60)
    if all_pass:
        print(
            f"{BOLD}✅ SUCCESS: All tests passed! Kruskal's algorithm is generic.{RESET}"
        )
    else:
