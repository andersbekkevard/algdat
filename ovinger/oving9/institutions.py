...
class HigherEdSolver:
    def initialize(self, institutions):
        self.parents = {institution: institution for institution in institutions}

    def parent_institution(self, institution):
        old_parent = self.parents[institution]
        if old_parent != institution:
            self.parents[institution] = self.parent_institution(old_parent)

        return self.parents[institution]
    ...
    for _ in range(k):
        ...
        while len(unis) != n_uni:
            unis.add(
                ...
                + "".join(
                    random.choices(
                        "ABCDEFGHIJKLMNOPQRSTUVWXYZ", k=math.ceil(math.log(n_uni, 20))
                    )
                )
            )
        active = set(unis)
