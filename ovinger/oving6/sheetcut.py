...
def solve_memo(memo: dict[tuple[int, int], float], w, h, p):
    if w == 0 or h == 0:
        return 0
    if (w, h) in memo:
        return memo[(w, h)]
    ...
    for i in range(1, w // 2 + 1):
        best = max(best, solve_memo(memo, w - i, h, p) + solve_memo(memo, i, h, p))
    for i in range(1, h // 2 + 1):
        best = max(best, solve_memo(memo, w, h - i, p) + solve_memo(memo, w, i, p))

    memo[(w, h)] = best
    return memo[(w, h)]


def solve_table(w, h, p):
    ...
    for i in range(1, h + 1):
        for j in range(1, w + 1):
            ...
            for cut in range(1, j):
                best = max(best, table[i][j - cut] + table[i][cut])
            for cut in range(1, i):
                best = max(best, table[i - cut][j] + table[cut][j])
            table[i][j] = best
...
def sheet_cutting(w, h, p):
    return solve_table(w, h, p)


def compare_algorithms():
    """Compare solve_memo and solve_table algorithms with their internal states"""
    # Simple test cases
    test_cases = [
        ({(1, 1): 1}, 1, 1),
        ({(1, 1): 1, (2, 1): 3, (1, 2): 3, (2, 2): 3}, 2, 2),
        ({(1, 1): 1, (2, 1): 1, (1, 2): 1, (2, 2): 5}, 2, 2),
        ({(1, 1): 1, (2, 1): 2, (1, 2): 2, (3, 1): 4, (2, 2): 3, (3, 2): 7}, 3, 2),
    ]

    for i, (prices, w, h) in enumerate(test_cases):
        print(f"\n{'='*60}")
        print(f"TEST CASE {i+1}: w={w}, h={h}")
        print(f"Prices: {prices}")
        print(f"{'='*60}")

        # Run solve_memo
        memo = {}
        result_memo = solve_memo(memo, w, h, prices)
        print(f"\nMEMOIZATION RESULT: {result_memo}")
        print("Memo state:")
        ...
        result_table = table[h][w]
        print(f"\nTABULATION RESULT: {result_table}")
        print("Table state:")
        for i in range(h + 1):
            row = []
            for j in range(w + 1):
                row.append(f"{table[i][j]:3d}")
            print(f"  Row {i}: {' '.join(row)}")

        # Compare results
        if result_memo == result_table:
            print(f"\n✓ RESULTS MATCH: {result_memo}")
        else:
            print(f"\n✗ RESULTS DIFFER: memo={result_memo}, table={result_table}")
...
def run_tests():
    ...
    if use_extra_tests:
        with open(current_dir / "tests_sheet_cutting.txt") as extra_tests_data:
            extra_tests = []
