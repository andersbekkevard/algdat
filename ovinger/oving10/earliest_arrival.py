...
def earliest_arrival(timetable, start, goal):
    # Track earliest arrival time at each station
    times = {}

    # Build adjacency list: station -> list of (destination, depart_time, arrive_time)
    trains = {}
    for a, b, t_0, t_1 in timetable:
        times[a] = times.get(a, float("inf"))
        times[b] = times.get(b, float("inf"))
        trains[a] = trains.get(a, []) + [(b, t_0, t_1)]
    ...
    while not Q.empty():
        current_time, a = Q.get()

        # Skip if we've already processed this with better time
        if current_time > times[a]:
            continue
        ...
        if a in trains:
            for b, t_0, t_1 in trains[a]:
                if current_time <= t_0:
                    if t_1 < times[b]:
                        times[b] = t_1
                        Q.put((t_1, b))

    return times.get(goal, float("inf"))
    ...
    for _ in range(k):
        ...
        while len(stations) < ns:
            stations.add(
                "".join(
                    random.choices(
                        "ABCDEFGHIJKLMNOPQRSTUVWXYZ", k=math.ceil(math.log(ns, 20))
                    )
                )
            )
        ...
        for _ in range(n):
            ...
            T.append((*random.sample(stations, k=2), t0, t1))
