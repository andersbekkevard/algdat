...
class Task:
    ...
    def __str__(self):
        depends_on_str = ", ".join("Task " + str(task.i) for task in self.depends_on)
...
def building_time_old(tasks: list[Task]):
    def find_time(task: Task):
        return max([find_time(dep) for dep in task.depends_on] + [task.time])

    return max(find_time(task) for task in tasks)
...
def building_time(tasks):
    ...
    for task in sorted_tasks:
        t = task.time
        if task.depends_on:
            t += max(times[dep] for dep in task.depends_on)
...
def gen_examples(k, nl, nu):
    for _ in range(k):
        ...
        shuffled_tasks = [
            (tasks[M[i]][0], [MD[j] for j in tasks[M[i]][1]]) for i in range(n)
        ]
        tasks = test_case_from_list(shuffled_tasks)
        yield tasks, max(D)


tests = [(test_case_from_list(test_case), answer) for test_case, answer in tests]
